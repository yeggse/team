...
//	public String menuAdd(HttpServletRequest request, HttpServletResponse response, Model model,
...
//			HttpServletRequest request, HttpServletResponse response, Model model) { // "file1"의 이름을 multi 로 하겠다는 의미
//		String url = null;
//		try {
//			String originFilename = multi.getOriginalFilename(); // 파일 이름
//			String extName = originFilename.substring(originFilename.lastIndexOf("."), originFilename.length()); // 파일
//																													// 확장자
...
//				map.put("img", "\\img\\menuImg\\" + saveFileName); // 경로+파일명 DB 저장 => xml 파일에서도 #{img} 로 맞추어 줘야
//				map.put("idx", idx); // 게시글 번호 DB저장
//				bService.insertMenuImg(map);
//
